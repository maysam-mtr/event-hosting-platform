...
export default function LatestMaps() {
  ...
  useEffect(() => {
    const fetchMaps = async () => {
      try {
        setIsLoading(true)
        const response = await getLatestMaps()
        setMaps(response.data)
        setFilteredMaps(response.data)
      } catch (error) {
        toast({
          title: "Error",
          description: "Failed to fetch latest maps. Please try again later.",
          variant: "destructive",
        })
      } finally {
        setIsLoading(false)
    ...
    <main className="container mx-auto px-4 py-8">
      ...
      {isLoading ? (
        ...
        <div className="text-center py-12">
          <h2 className="text-xl font-semibold mb-2">No latest maps found</h2>
